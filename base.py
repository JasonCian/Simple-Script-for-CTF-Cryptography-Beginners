...
def base16_encrypt(message):
    print("base16加密结果: ", end="")
    print(base64.b16encode(message))

def base32_encrypt(message):
...
def base16_decrypt(message):
    print("base16解密结果: ", end="")
    print(base64.b16decode(message))

def base32_decrypt(message):
...
def getTranslatedMessage(mode, message, typel):
    if mode[0] == 'e':
        if typel == 16:
            base16_encrypt(message)
        elif typel ==32:
            base32_encrypt(message)
        elif typel ==64:
            base64_encrypt(message)
    else:
        if typel == 16:
            base16_decrypt(message)
        elif typel ==32:
            base32_decrypt(message)
        elif typel ==64:
            base64_decrypt(message)
